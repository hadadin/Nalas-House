...
import { getHouseholdId, getAccount, getTasks, getLatestMenuId } from "@/lib/queries";
...
export default async function HomePage() {
  ...
  // These are cached — layout already called them, no extra DB round-trips
  const [account, tasks, menuId] = await Promise.all([
    getAccount(hh.householdId),
    getTasks(hh.householdId),
    getLatestMenuId(hh.householdId),
  ]);

  const todayIndex = dayOfWeekIndex(new Date());
  let todayMeals: Meal[] = [];
  if (menuId) {
    const { data: mealData } = await supabase
      .from("meal")
      .select("id, menu_id, day_index, meal_type, name, cook_time_minutes, steps, ingredient(id, meal_id, name, quantity, unit, aisle)")
      .eq("menu_id", menuId)
      .eq("day_index", todayIndex);
    todayMeals = (mealData ?? []) as Meal[];
  }
  ...
  return (
    <div style={{ padding: "20px 20px 24px", display: "flex", flexDirection: "column", gap: 26 }}>
      <div>
        <div style={{ fontSize: 12, color: "var(--coral)", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 6 }}>The week ahead</div>
        <div className="serif" style={{ fontSize: 30, lineHeight: 1.1, color: "var(--ink)" }}>
          <span style={{ fontWeight: 700 }}>Good </span>
          <span style={{ fontStyle: "italic", fontWeight: 500 }}>morning, {account.user_name}.</span>
        </div>
      </div>

      {/* Yellow hero CTA */}
      <div style={{ background: "var(--brand)", borderRadius: 20, padding: "20px 22px" }}>
        <div className="serif" style={{ fontSize: 22, fontWeight: 700, color: "var(--on-brand)", lineHeight: 1.15, maxWidth: 260 }}>
          Plan your week <span style={{ fontStyle: "italic", fontWeight: 500 }}>in 30 seconds.</span>
        </div>
        ...
        <Link href="/food" style={{ display: "inline-block", background: "var(--green)", color: "var(--on-green)", borderRadius: 999, padding: "10px 20px", fontSize: 13, fontWeight: 700, textDecoration: "none" }}>
          {menuId ? "View this week's menu" : "Generate this week's menu"}
        </Link>
      </div>
              ...
              return (
                <div key={mt} style={{ display: "flex", alignItems: "center", gap: 14, padding: "10px 0", borderBottom: i < 2 ? "1px solid var(--line)" : "none" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {meal?.cook_time_minutes && <div style={{ fontSize: 11, color: "var(--ink3)", marginTop: 1 }}>{meal.cook_time_minutes} min</div>}
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {upcomingTasks.map((task) => (
              <div key={task.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: "var(--surface)", borderRadius: 14, border: "1px solid var(--line)" }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: "var(--green)", background: "var(--green-soft)", borderRadius: 999, padding: "3px 10px" }}>{task.assignee}</span>
              </div>
            ))}
          </div>
        </div>
